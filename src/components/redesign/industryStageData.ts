...
export const TRADES_STAGE: IndustryStageData = {
  ...
  hero: {
    ...
    intro:
      "The admin your office team shouldn't be doing — missed calls, qualifying, quote chasing, paperwork, follow-up — built around the job management system you already run, and run by Rosebud.",
  },
  stage1: {
    index: "01",
    eyebrow: "Win the job",
    h2: "How do you stop losing new jobs to missed calls?",
    answer:
      "You answer every inquiry in under a minute, on every channel. A missed call rings out and a text lands seconds later with a booking link; web-form and WhatsApp enquiries get an immediate reply. The 7am boiler emergency reaches you first — with the type of work, urgency and access already captured.",
    items: [
  ...
  stage2: {
    index: "02",
    eyebrow: "Run the job",
    h2: "Who chases the quotes and paperwork while you're on the tools?",
    answer:
      "The system does, in the background. Every quote is followed up on a set cadence, deposit invoices and sign-offs are collected in conversation rather than chased by email, and customers get status updates at each stage — so the office only steps in when a job genuinely needs a person.",
    cards: [
  ...
  pullStat: { pre: "Every quote followed up on a set cadence — ", accent: "nothing left to chase from memory." },
  stage3: {
    index: "03",
    eyebrow: "Get paid & repeat",
    h2: "How do you get paid faster and win the repeat work?",
    answer:
      "Invoices go out automatically the moment a job is done, with reminders on the cadence that gets them paid. Then every customer is followed up at three, six and twelve months — so repeat work and referrals stop depending on someone in the office remembering to pick up the phone.",
    items: [
  ...
  faqs: [
    ...
    { q: "Does it work with the job management system we already use?", a: "Yes — it's built around the job management system you already run, which stays your source of truth. Rosebud writes into it and reads from it; it feeds your system rather than replacing it." },
    { q: "How do you make sure the system launches safely?", a: "Every build is scoped to how your business already runs, tested against real enquiries, and goes live with your sign-off at each stage. Nothing is switched on until it behaves the way your office would." },
  ],
  related: [
  ...
  schema: {
    ...
    serviceDescription:
      "A CRM and job-intake layer for plumbing, HVAC and home-service contractors that answers inquiries in under a minute, qualifies and books jobs against the live diary, chases quotes, files paperwork and runs customer follow-up — built around the job management system the business already runs.",
