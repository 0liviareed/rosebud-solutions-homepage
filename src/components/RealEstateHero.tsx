...
// Rosebud Solutions — Real Estate hero (animated workflow).
// Same template as the other industry heroes, content swapped to the
// real-estate desk. Design-system layer: Cormorant / DM Sans, transparent over
// the page background, system purple, glow BookDemoCTA. Stops at "showing
// booked / routed"; the offer and the close are the agent's.
...
const STAGES: Stage[] = [
  {
    tab: "Inquiry",
    inputs: [
      { label: "Lead channels", detail: "Zillow, Realtor.com, IDX, SMS" },
      { label: "Qualification", detail: "Budget, pre-approval, timeline" },
      { label: "Routing rules", detail: "Luxury, first-time, investor" },
    ],
    output: {
      chip: "Lead qualified",
      q: "What happens to the Zillow lead that comes in at 9pm?",
      a: "Answered in seconds, qualified on budget, pre-approval and timeline, and routed to the right agent. The hot ones to a closer, the long-tail to nurture.",
      metric: { value: "< 60s", label: "response to every lead" },
    },
  },
  {
    tab: "Showings",
    inputs: [
      { label: "Agent calendars", detail: "Availability across the team" },
      { label: "Live CRM", detail: "Follow Up Boss, kvCORE, Sierra" },
      { label: "Reminder cadence", detail: "Confirmations, reschedules" },
    ],
    output: {
      chip: "Showing booked",
  ...
  },
  {
    tab: "Nurture",
    inputs: [
      { label: "Dormant leads", detail: 'The "maybe next spring" list' },
      { label: "Engagement signals", detail: "Opens, replies, activity" },
      { label: "Matching listings", detail: "New inventory that fits" },
    ],
    output: {
      chip: "Long-tail kept warm",
      q: "Who's coming back that you'd have dropped?",
      a: "The 6-to-12-month buyers kept in a real conversation across text, email and voice, with listings that match, routed back to your closer the moment a buying signal fires.",
      metric: { value: "6–12 mo", label: "nurture that resumes when they're ready" },
    },
  },
];
    ...
    <section className="rbh">
      ...
      <div className="rbh-eyebrow">
        ...
        <span className="rbh-loc">
          <i>IV</i> By Industry · Real Estate
        </span>
      </div>

      <h1 className="rbh-h1">
        First one back wins. <em>Now that&apos;s you.</em>
      </h1>

      <p className="rbh-sub">
        Every lead answered, qualified, and booked into a showing in under 60
        seconds. Even the 9pm Zillow inquiry you&apos;d have called back Monday.
      </p>
      ...
      <div
        className="rbw-tabs"
        role="tablist"
        aria-label="Real estate workflow stages"
        onMouseEnter={hold}
