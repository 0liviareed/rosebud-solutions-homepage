...
// Rosebud Solutions — Family Law & Consumer Legal hero (animated workflow).
// Same template as the other industry heroes, content swapped to the small-firm
// legal desk. Compliance posture held strictly: the system captures conflict
// inputs but never runs the check, the attorney accepts every matter, status is
// factual/procedural only, and substantive questions route to the attorney. No
// legal advice, no substantive drafting, no case-outcome claims anywhere here.
// Design-system layer: Cormorant / DM Sans, transparent over the page
// background, system purple, glow BookDemoCTA.
...
const STAGES: Stage[] = [
  ...
  },
  {
    tab: "Deadlines",
    inputs: [
      { label: "Court deadlines", detail: "Computed off triggers" },
      { label: "Documents + records", detail: "Client docs, third-party requests" },
      { label: "Scheduling", detail: "Depositions, mediations, parties" },
    ],
    output: {
  ...
  },
  {
    tab: "Billing",
    inputs: [
      { label: "Real-time capture", detail: "Not reconstructed month-end" },
      { label: "Status updates", detail: "Factual, procedural only" },
      { label: "Invoicing + chase", detail: "On schedule, automated" },
    ],
    output: {
      chip: "Time captured, billed",
      q: "Where do the billable hours leak?",
      a: "Time captured as the work happens instead of reconstructed at month-end, factual status updates sent before the client calls, invoices out on schedule with payment chased automatically. Substantive questions route to you.",
      metric: { value: "Real time", label: "captured as work happens, not month-end" },
    },
  },
];
    ...
    <section className="rbh">
      ...
      <div className="rbh-eyebrow">
        ...
        <span className="rbh-loc">
          <i>VII</i> By Industry · Family Law &amp; Consumer Legal Practices
        </span>
      </div>

      <h1 className="rbh-h1">
        We don&apos;t replace your judgment.{" "}
        <em>We replace the admin around it.</em>
      </h1>

      <p className="rbh-sub">
        Intake. Deadlines. Document chase. Status updates. Billing. Handled by a
        system we build around your firm. No drafting. No advice. No conflict
        checks.
      </p>
      ...
      <div
        className="rbw-tabs"
        role="tablist"
        aria-label="Legal workflow stages"
        onMouseEnter={hold}
...
.rbh-loc{font-size:12px; letter-spacing:.2em; text-transform:uppercase; color:var(--muted); text-align:center;}
.rbh-loc i{font-family:var(--serif); font-style:italic; color:var(--accent); letter-spacing:0; margin-right:6px; text-transform:none;}

.rbh-h1{font-family:var(--serif); font-weight:500; font-size:clamp(33px,5.8vw,70px); line-height:1.06; letter-spacing:-.018em; margin:0 auto 28px; max-width:17ch;}
